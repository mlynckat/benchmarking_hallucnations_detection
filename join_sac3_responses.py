...
def join_with_extended():
    for dataset in ["FAVA_chatgpt", "FAVA_llama", "FELM_math", "FELM_reasoning", "FELM_science", "FELM_wk", "FELM_writing_rec"]: #"FAVA_chatgpt", "FAVA_llama",
        ...
        df_updated["sac3_vote(all)"] = df_updated["sc2_vote"] + df_updated["sac3_q_vote"] + df_updated[
            "sac3_qm(starling)_vote"]
        df_updated["sac3_score(all)"] = df_updated["sac3_vote(all)"].apply(lambda x: sum(x) / len(x))
        df_updated["sac3_vote(all_best)"] = df_updated["sc2_vote"] + df_updated["sac3_qm(starling)_vote"]
        df_updated["sac3_score(all_best)"] = df_updated["sac3_vote(all_best)"].apply(lambda x: sum(x) / len(x))
        df_updated["sac3_vote(all_no_perb)"] = df_updated["sc2_vote"] + df_updated["starling_self"]
        df_updated["sac3_score(all_no_perb)"] = df_updated["sac3_vote(all_no_perb)"].apply(lambda x: sum(x) / len(x))
        df_updated["sac3_vote(all)_short"] = df_updated["sc2_vote_short"] + df_updated["sac3_q_vote_short"] + df_updated[
            "sac3_qm(starling)_vote_short"]
        df_updated["sac3_score(all)_short"] = df_updated["sac3_vote(all)_short"].apply(lambda x: sum(x) / len(x))
        print(df_updated["sac3_score(all)_short"])
        print(df_updated.columns)
