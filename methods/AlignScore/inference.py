...
class Inferencer():
    def __init__(self, ckpt_path, model='bert-base-uncased', batch_size=32, device='cuda', verbose=True) -> None:
        ...
        if ckpt_path is not None:
            self.model = BERTAlignModel.load_from_checkpoint(checkpoint_path=ckpt_path, strict=False, model=model).to(
                self.device)
        else:
    ...
    def inference_example_batch(self, premise: list, hypo: list):
        ...
        out_score = []
        for one_pre, one_hypo in tqdm(zip(premise, hypo), desc="Evaluating", total=len(premise),
    ...
    def inference(self, premise, hypo):
        ...
        for mini_batch in tqdm(batch, desc="Evaluating",
    ...
    def batch_tokenize(self, premise, hypo):
        ...
        batch = []
        for mini_batch_pre, mini_batch_hypo in zip(self.chunks(premise, self.batch_size),
                                                   self.chunks(hypo, self.batch_size)):
            try:
                mini_batch = self.tokenizer(mini_batch_pre, mini_batch_hypo, truncation='only_first',
                                            padding='max_length', max_length=self.tokenizer.model_max_length,
                                            return_tensors='pt')
            except:
                warning('text_b too long...')
                mini_batch = self.tokenizer(mini_batch_pre, mini_batch_hypo, truncation=True, padding='max_length',
    ...
    def smart_doc(self, premise: list, hypo: list):
        ...
        for one_pre, one_hypo in tqdm(zip(premise, hypo), desc="Evaluating SMART", total=len(premise)):
            out_score.append(
                self.smart_l(one_pre, one_hypo)[1] if self.smart_type == 'smart-l' else self.smart_n(one_pre, one_hypo)[
                    1])
    ...
    def smart_n(self, premise, hypo):
        ...
        prec = sum([max([sum([output_score[i + n, j + n] / n_gram for n in range(0, n_gram)]) for i in
                         range(len(premise_sents) - n_gram + 1)]) for j in range(len(hypo_sents) - n_gram + 1)])
        ...
        recall = sum([max([sum([output_score[i + n, j + n] / n_gram for n in range(0, n_gram)]) for i in
                           range(len(premise_sents) - n_gram + 1)]) for j in range(len(hypo_sents) - n_gram + 1)])
